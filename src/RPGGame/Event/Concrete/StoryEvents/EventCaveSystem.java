...
public class EventLostCave extends StoryEvent {
    EventLostCave(){
        ...
        this.setReencounterText("""
                The cave's twists and turns seem endless. Will you ever find your way out?
                """);
        this.addChoice(new Choice("Search for a way out") {
            @Override
            public void execute(Player player, PrimaryScene scene) {
                scene.gameOutput("You begin to explore the cave, hoping to find an exit. The moss-covered walls and eerie silence make your journey unsettling.");
                // Implement consequences of searching for a way out (e.g., encountering obstacles, finding clues)
            }
        });
